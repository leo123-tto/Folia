...
- 导出预设管理坚持低心智负担：Settings / Word 导出采用二级页面承载 `预设库`、`自定义槽位`、`JSON 示例`，避免把内置预设、自定义槽位、JSON 示例和纸张预览全部堆在首屏；不提供字体、页边距等复杂表单编辑器。Word 纸张预览只在 `预设库` 中显示，可点击放大查看，其余二级页用全宽内容区承载槽位和示例。
- Settings / HTML 导出复用同一信息架构：`预设库` 展示少量通用 HTML 主题和启用状态，`自定义槽位` 承载 2 个常规 CSS 预设槽位和 CSS 预设导入，`CSS 示例` 只说明安全文章选择器范围；小型 HTML 文章预览只在 `预设库` 和 `自定义槽位` 中显示，不占用 `CSS 示例` 的全宽阅读空间。
