...
| Toolbar 图标按钮 | 34px 容器 / 19px icon | — | lucide icon | 打开、保存、另存、源码、Word 预览、大纲、设置 |
...
- 高度 44px，**透明背景**（融入内容区）
- 左侧：icon-only 文件操作（打开、保存、另存）+ 文件名（文件名默认 `opacity: 0`，打开文件后渐入）；主界面不显示 Folia 应用名
- 右侧：源码模式切换 + Word 预览开关 + icon-only 大纲开关 + Settings 按钮
- 图标语义统一为“文件流转”：打开使用 file input，保存使用 file check，另存为使用 files；源码、Word 预览、大纲、设置分别使用 square code、file search、list collapse、sliders，避免齿轮/书本/代码标签带来的普通按钮感
- macOS 原生窗口标题栏使用 overlay，不显示独立系统标题条；红黄绿按钮浮在 WebView 左上角，Toolbar 在 Tauri + macOS 环境下预留 92px 左侧空间
- Toolbar 背后和左右控件之间的空白都必须是 `data-tauri-drag-region`，并挂载手动 `startDragging()` fallback；按钮本身保持可点击，双击空白区域触发窗口最大化切换
- `user-select: none`
...
- 背景 `--bg`（暖调奶油色）
- 内容容器默认 max-width 680px，可在 Settings 中选择 640 / 680 / 720 / 800px；居中，padding `36px 40px 80px`
- 原生 HTML table 阅读预览使用更宽版心 `min(1120px, 100% - 72px)`，优先保证宽表可读
...
- 普通 Markdown 默认是 Typora-like 所见即所得编辑区，Markdown 标记弱化或按上下文显示。
- 检测到原生 `<table>` 或打开 `.html` 文件时，主内容区自动切换为稳定 HTML 阅读预览，不强制进入 WYSIWYG；源码模式作为复杂 HTML table 的编辑入口和排障 fallback。
...
- 导出预设管理坚持低心智负担：Settings / 导出只提供预设选择、JSON 导入、模板复制和删除自定义预设，不提供字体、页边距等复杂表单编辑器。
...
TOC 大纲面板默认宽度为 224px，位于主内容区左侧。标题使用 10.5px mono，条目使用 13px / 20px 行高，确保长法律文档中大纲可读，但仍低于正文视觉层级。

后续如加入 Command Palette 大纲命令，TOC 仍可保留为可选常驻导航，不应默认占用复杂 HTML 表格的横向阅读空间。
...
| Preview 内容区 | `36px 40px 80px`；HTML table 阅读预览使用更宽版心 `min(1120px, 100% - 72px)` |
...
- 导出预设不做复杂编辑器；用户新增样式通过 JSON 文件导入，Settings 只提供导入、复制模板、删除自定义预设和选择当前预设。
...
- **保存后**：dirty 标记消失
- **发现更新**：启动后延迟检查；如果有新版本，只显示一个克制的居中确认弹窗，不在 Toolbar 常驻更新提示

### 页面切换

- **编辑器页面**：默认页面，Title Bar + WYSIWYG Editor 或 Stable HTML Reading + 可选 Word Preview + Status Bar
- **设置页面**：固定尺寸弹窗，不让主窗口尺寸随二级菜单内容跳动；“关于”页承载版本号、自动检查更新开关、手动检查更新按钮和更新源信息
