...
暖调奶油底色（`oklch(97% 0.012 80)`）营造纸张质感，默认 WYSIWYG 写作区也使用同一暖底，避免中间出现突兀白块。只有 Word 纸张预览使用接近白色的 `--surface` 来表达真实 A4 纸张。Markdown 阅读标题默认跟随阅读正文字体，不再按 H1/H2 与 H3/H4 混用衬线和非衬线；层级依靠字号、间距和 520-600 的渐进字重建立，保证中文长文和中英文混排稳定。
...
| 角色 | 变量 | 字体 | 回退 | 用途 |
|------|------|------|------|------|
| Display | `--font-display` | `'Iowan Old Style'` | `'Charter'`, `'Songti SC'`, `'STSong'`, `'Noto Serif CJK SC'`, `'Source Han Serif SC'`, `'SimSun'`, Georgia, serif | 标题、设置页标题 |
| Body / UI | `--font-body` | `-apple-system` | `BlinkMacSystemFont`, `'Segoe UI'`, `'Microsoft YaHei UI'`, `'PingFang SC'`, system-ui, sans-serif | UI 文字 |
| Reading | `--font-reading` | `-apple-system` | `BlinkMacSystemFont`, `'Segoe UI'`, `'PingFang SC'`, `'Microsoft YaHei'`, `'Noto Sans CJK SC'`, `'Source Han Sans SC'`, sans-serif | Markdown 阅读正文、即时渲染编辑正文 |
| Reading Override | `--reading-font-family` / `--preview-font-family` | Settings 生成 | 英文字体栈 → 中文字体栈 → 默认 reading 栈 | Markdown 阅读和即时渲染编辑的用户字体选择 |
| Reading Heading | `--reading-heading-font-family` / `--preview-heading-font-family` | 默认跟随正文 | 可选跟随中文、英文、系统衬线或自定义标题字体 | H1-H6 统一标题字体 |
| Serif Reading | `--font-serif-reading` | `'Songti SC'` | `'STSong'`, `'Noto Serif CJK SC'`, `'Source Han Serif SC'`, `'SimSun'`, `'Times New Roman'`, serif | 中文宋体阅读预设 |
| Mono | `--font-mono` | `ui-monospace` | `'SF Mono'`, `'Cascadia Mono'`, `'JetBrains Mono'`, `'IBM Plex Mono'`, Menlo, Consolas, `'PingFang SC'`, monospace | 源码编辑器、状态栏、文件名 |
...
| H1 | 28px | 600 | reading heading | 一级标题 |
| H2 | 22px | 580 | reading heading + border-bottom | 二级标题 |
| H3 | 18px | 560 | reading heading | 三级标题 |
| 预览正文 | 15px | 400 | reading | line-height 1.7 |
| 预览表格 | 14px | 400 | reading | 表格内容 |
...
- **标题字体统一**：Markdown H1-H6 默认跟随阅读正文，不按层级混用衬线/非衬线；用户选择标题字体时也覆盖所有标题层级。
- **字重做层级**：标题 weight 520-600，正文 400，靠字重、字号和间距区分。
- **中文/英文可分选**：Settings / 预览提供中文字体、英文字体和标题字体选择；默认选项保持克制，不再暴露“中文优化 / 中文宋体”作为主路径。
...
- WYSIWYG / Live Preview 主显示面上下 padding 控制在 10px，尽量把 Markdown 内容区域贴近 Toolbar 与 Status Bar 之间的完整高度
...
- 源码模式下浮动 TOC 点击标题必须滚动 CodeMirror 到对应 Markdown 标题行，不能只更新大纲高亮状态

### Preview Pane

- 背景 `--bg`（暖调奶油色）
- 内容容器默认 max-width 680px，可在 Settings 中选择 640 / 680 / 720 / 800px；居中，padding `10px 40px 12px`
- 原生 HTML table 阅读预览使用更宽版心 `min(1120px, 100% - 72px)` 和更紧凑的 `10px 28px 12px` padding，优先保证宽表可读和大文档可视高度
...
- 检测到原生 `<table>` 或打开 `.html` 文件时，主内容区自动切换为稳定 HTML 阅读预览，不强制进入 WYSIWYG；稳定预览顶部提供轻量“编辑表格”和“编辑源码”入口。“编辑表格”打开结构化 modal，只替换单个 table block；源码模式作为复杂 HTML table 的高级入口和排障 fallback。对于包含原生 HTML table 的 Markdown 文档，稳定预览顶部还提供“退出 HTML 预览”，允许用户临时回到普通 Markdown / WYSIWYG 预览继续阅读大文件；普通 Markdown 预览顶部保留“HTML 阅读预览”入口用于切回。`.html/.htm` 文件不提供退出到 Markdown 预览，避免把完整 HTML 文件交给 WYSIWYG。
...
- “HTML 预览”与 Word 预览并列作为右侧互斥导出面板入口。面板标题使用 HTML 预览 / HTML 导出语义，复制按钮文案保留“复制到公众号编辑器”，避免把整个设置体系命名为公众号。
- 导出预设管理坚持低心智负担：Settings / Word 导出采用二级页面承载 `预设库`、`自定义槽位`、`JSON 示例`，避免把内置预设、自定义槽位、JSON 示例和纸张预览全部堆在首屏；三个页签必须作为等宽铺满整行的分段式三级选项框，不提供字体、页边距等复杂表单编辑器。Word 纸张预览只在 `预设库` 中显示，可点击放大查看，预览侧只显示预设名；自定义槽位页不提供顶部导入按钮，用户点击空槽位导入 JSON。
- Settings / HTML 导出复用同一信息架构：`预设库` 展示少量通用 HTML 主题和启用状态，`自定义槽位` 承载 2 个常规 CSS 预设槽位，只通过点击空槽位导入 `.css` 样式文件或 `.json` 预设文件，不提供设置页内手写 CSS 表单；`CSS 示例` 只展示可选中复制的 CSS 示例文本；三个页签同样使用等宽铺满整行的分段式三级选项框。小型 HTML 文章预览只在 `预设库` 中显示并支持点击放大，预览侧只显示预设名，且缩略宽度要给左侧列表保留呼吸感。HTML 内置预设条目不显示主题来源，用户只需要理解它们是 Folia 内置预设。
- Word / HTML 导出设置页不在顶部操作区提供“删除/停用”总按钮；预设启用/停用和自定义预设删除都放回每个预设条目的右侧控制，减少重复入口。
...
TOC 不再作为顶部按钮出现。文档存在标题时，内容区左侧默认显示一列极弱的浮动横线刻度，表示大纲存在但不抢占阅读空间；起点控制在主内容区顶部下方约 48px，跟随更紧凑的 Markdown 显示面。鼠标移入、点击或键盘聚焦横线轨道时向右展开半透明标题列表；横线轨道只负责轻量查看大纲，不直接切换固定状态。展开面板右上角提供明确的固定按钮和关闭按钮：固定按钮把大纲变为左侧常驻栏并让正文区域让位，关闭按钮收起面板并回到轻量横线入口。固定状态下按钮切换为取消固定，关闭按钮则直接收回到轻量横线状态；“总是固定大纲”只在固定态左侧栏中出现，作为持久开关控制后续含标题文档是否默认固定。未固定时，从横线轨道移动到展开面板不能出现 hover 断层，面板条目必须可点击。长标题文档下，折叠轨道和展开列表都必须限制在状态栏上方并允许内部滚动。

浮动 TOC 条目使用 13px / 19px 行高，当前标题使用 accent 色高亮；未展开状态只显示 18px 以内的刻度线，命中区域也限制在轨道附近，不能形成覆盖正文的透明面板。固定态占用左侧独立栏位并挤出正文空间，Word 预览面板打开时仍保持在左侧，避免遮挡导出预览。

### 授权与高级槽位

授权入口不直接塞进 Word 导出页面，而是在 Settings 侧栏中作为独立“授权”页面出现。Word 导出里的内测授权槽位只提供前往授权页的轻量入口。

授权页第一阶段以内测码为主：输入内测码、显示授权状态和可用自定义预设槽位数。内测码只用于开启本机额外自定义槽位；锁定槽位只提供前往授权页的轻量入口。授权校验不得阻塞打开文档；已激活用户在离线状态下仍可使用本地缓存的额外槽位。
...
- 内容字体、正文颜色、标题层级、链接颜色、段落缩进、列表、引用、代码块、行内代码、表格字号/行高/字体颜色、分割线和图片最大宽度直接读取 Word 导出 `PresetConfig` 或导出端固定映射，避免“预览看起来对、导出变了”的割裂。
- 预览内容源采用快速 Markdown HTML：右侧 Word 预览通过 Vditor 将当前 Markdown 渲染为 HTML，再由 Word 导出 `PresetConfig` 映射出的 CSS 纸张规则控制 A4、页边距、字体、标题、正文、表格和图片比例；默认预览不得生成临时 `.docx`、调用外部转换器或嵌入 PDF。
...
- 原生 HTML table / `.html` 文件：默认稳定阅读预览；工具栏源码按钮切换到 CodeMirror 源码模式；Markdown 文档中的原生 HTML table 可通过“退出 HTML 预览”临时回到普通 Markdown 预览
...
| 平台 | WebView | Display 字体回退 | Reading 字体回退 |
|------|---------|-----------------|--------------|
| macOS | WKWebView | Iowan Old Style → Songti SC | PingFang SC |
| Windows | WebView2 (Chromium) | Charter → SimSun / Noto Serif CJK SC | Microsoft YaHei |
| Linux | WebKitGTK | Georgia → Noto Serif CJK SC | Noto Sans CJK SC / Source Han Sans SC |
...
- **打开文件**：文件名渐入；普通 Markdown 进入 WYSIWYG，原生 HTML table / `.html` 文件进入稳定阅读预览；含原生 HTML table 的 Markdown 可手动退出到普通 Markdown 预览；Word 预览仍需用户主动打开，Status Bar 显示文件路径
- **编辑中**：编辑器修改同步到 Markdown 源码；Word 预览打开时 debounce 更新，dirty 标记出现
- **保存后**：dirty 标记消失
- **发现更新**：用户启用自动检查时启动后延迟检查；如果有新版本，后台静默下载，不显示阻塞弹窗或下载进度；下载完成后在源码按钮左侧显示“重启更新”
...
官方网站不是桌面应用主界面，但必须延续 Folia 的品牌气质：暖调纸面、衬线标题、克制工具感和单一赭色强调。官网允许比桌面应用更强的产品展示和更大的标题层级，但不使用紫蓝渐变、营销式卡片堆叠或纯装饰光斑。

- 首屏必须直接呈现 Folia 名称、产品定位、下载入口和可识别的应用界面预览。
- 官网可使用 `--sage`、`--steel` 等低饱和辅助色做细小分区提示，但主强调仍然是 `--accent`。
- 产品界面预览应表达真实能力：Markdown、复杂表格、Word 纸张预览；不使用无关抽象插画替代产品本身。
- 移动端优先保证标题、按钮和下载说明不溢出；应用预览可作为背景信号弱化，不遮挡主要文案。
