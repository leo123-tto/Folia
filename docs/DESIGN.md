...
- 预览内容源应优先来自真实导出产物：右侧 Word 预览先生成临时 `.docx`，本机 LibreOffice renderer 可用时嵌入其 headless 导出的 PDF；不可用时再从该产物转换出预览 HTML，现有 CSS 纸张外壳负责缩放、分页和 fallback，而不是直接以 Markdown/Vditor 渲染结果作为最终预览源。
