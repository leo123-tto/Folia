...
- 导出预设管理坚持低心智负担：Settings / Word 导出采用二级页面承载 `预设库`、`自定义槽位`、`JSON 示例`，避免把内置预设、自定义槽位、JSON 示例和纸张预览全部堆在首屏；三个页签必须作为等宽铺满整行的分段式三级选项框，不提供字体、页边距等复杂表单编辑器。Word 纸张预览只在 `预设库` 中显示，可点击放大查看，其余二级页用全宽内容区承载槽位和示例。
- Settings / HTML 导出复用同一信息架构：`预设库` 展示少量通用 HTML 主题和启用状态，`自定义槽位` 承载 2 个常规 CSS 预设槽位和 CSS 预设导入，`CSS 示例` 只说明安全文章选择器范围；三个页签同样使用等宽铺满整行的分段式三级选项框。小型 HTML 文章预览只在 `预设库` 中显示并支持点击放大，`自定义槽位` 和 `CSS 示例` 都使用全宽内容区。HTML 内置预设条目不显示主题来源，用户只需要理解它们是 Folia 内置预设。
...
授权页第一阶段以内测码为主：输入内测码、显示授权状态、可用自定义预设槽位数、过期时间或永久授权状态。合规确认前不展示价格、购买按钮、订阅入口、商业分层或会员文案；锁定槽位只说明“常规版本可保存 2 个自定义预设，输入内测码可使用更多槽位”。授权校验不得阻塞打开文档；已激活用户在离线状态下仍可使用本地缓存的高级槽位权益。
