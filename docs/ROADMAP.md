...
- [ ] HTML 演示预览：直接打开受信任 HTML 演示文件，隔离运行其 JS/CSS/本地资源，并支持常见翻页操作
...
- [x] 明确高级槽位授权路线：先做内测码入口和可替换 `licenseService`，合规确认前不内置支付界面
...
- **2026-05-21**
  ...
  - Settings / HTML 导出新增 `预设库 / 自定义槽位 / CSS 示例` 二级页，支持启用/停用内置预设、保存 2 个常规自定义 CSS 槽位、CSS 预设交换 JSON 导入 / 导出；设置页预览仅在预设库显示并可点击放大，旧 `wechatCustomCss` 自动迁移为基于默认主题的自定义 HTML 预设。
...
- **2026-05-20**
  ...
  - 进一步收口商业化路径：取消商业版表达，高级槽位先采用内测码入口和本地/在线授权服务抽象，合规确认前不把价格页、购买入口或支付界面放进桌面端；Word 导出设置后续改为二级页面，锁定槽位跳转到独立授权页。
