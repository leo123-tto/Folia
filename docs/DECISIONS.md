...
**背景**
用户提出：导出预设本身虽然不是复杂功能，但对一部分用户可能有明确价值，未来可以作为付费功能或增值内容开发。进一步明确后，付费点不是售卖某个预设内容本身，而是解锁额外的 Word 导出自定义预设槽位。当前 Folia 已经支持内置预设和用户导入 JSON 自定义预设，这为后续槽位模型提供了基础。

**选项**
1. 将基础 JSON 预设导入改为付费。优点是实现直接；缺点是会削弱用户对工具的信任，也容易让一个轻量阅读器显得过度商业化。
2. 保持基础功能免费，提供少量自定义预设槽位；付费解锁更多或不限量槽位。优点是核心阅读和基础导出不受影响，付费点清晰、易理解；缺点是后续需要设计槽位计数、迁移和授权逻辑。
3. 暂不考虑商业化，只保留开源/本地导入。优点是简单；缺点是错过专业用户对规范化导出样式的付费意愿。

**决策**
选择方案 2 作为未来方向。

短期保持：
- 复杂 HTML 阅读、基础 Word 导出和内置预设不作为付费门槛。
- JSON 预设导入能力本身保留，但未来可对“可保存的自定义预设数量”设置免费上限。
- 当前本地 `custom:*` 预设注册表可自然承载槽位计数。

未来可探索：
- 免费用户保留少量自定义 Word 导出预设槽位，具体数量后续按产品测试确定。
- 付费解锁更多槽位或不限量槽位。
...
**理由**
Folia 的核心价值是轻量、可靠、打开即读。基础阅读和基础导出如果被付费墙挡住，会增加心智负担；而“保存更多自定义导出预设”属于专业用户的容量需求，边界更清晰，也不会影响普通用户完成核心任务。槽位模型比售卖单个预设内容更简单，更适合先做轻量商业化验证。

**影响**
- `docs/ROADMAP.md` 增加 v0.8 预设生态与商业化探索阶段。
- 后续实现付费能力时，应优先围绕“自定义预设槽位数量”而不是“基础导入按钮”设计。
- 设置页后续可能需要展示当前槽位用量，例如 `2 / 3`；超过免费槽位时引导解锁。
...
- **目标:** 记录用户提出的导出预设商业化方向
- **操作:** 在 `docs/ROADMAP.md` 增加 v0.8“预设生态与商业化探索”，并新增 DEC-014；随后根据用户进一步澄清，将商业化模型从“预设包/模板分发”修正为“付费解锁额外自定义 Word 导出预设槽位”。
- **结果:** 未来商业化方向已进入路线图和决策记录；当前版本不实现付费逻辑，也不影响核心阅读、基础导出和启动速度。
- **下一步:** 等产品形态更稳定后，再评估免费槽位数量、槽位用量展示、本地授权校验和团队槽位策略。
